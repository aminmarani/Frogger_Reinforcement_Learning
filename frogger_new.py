...
class Frogger(base.PyGameWrapper):
    def __init__(self, width=kScreenWidth, height=kScreenHeight):
        ...
        #self.frog_sprt = BaseSprite(self)  
        #self.frog_sprt = frog_sprites.BaseSprite(self)  
    ...
    def getScore(self): # change reward function
        return self.score

    def getGameState(self): # STATES...
        homeStatus=[None]*len(self.homes)
        for i in range(len(self.homes)):
            if self.homes[i].frogged == True: homeStatus[i] = float(0.66) # flying is worth more
    ...
    def get_car_speed(self):
        frog_sprt = frog_sprites.BaseSprite(self)  
        #spt = basesprt.get_speed()
        spt = frog_sprt.get_speed()
        return spt
    ...
    def stepping(self,dt):
        ...
        if self.frog._y < kPlayYHomeLimit:
            ...
            if collideInd != -1 and self.homes[collideInd].frogged == False and self.homes[collideInd].croced == False:
                ...
                if self.numFrogsHomed == 5: self.score = self.rewards['win']
                if self.frog_y == 261.0 : self.score = self.rewards['middle_way']
                self.frog.kill()
                self.frog = Frog(kPlayFrog, self.images["frog"]["stationary"])
                self.reachedMidway = False
            else:
